...
def worker(i):
    while True:
        ...
        Lx, Ly = eval(L)
        #Lx, Ly = L, L
        mean_corr_mat = np.zeros(shape=(Lx*Ly,Lx*Ly))
        for instance in range(len(reverse_dict_list)):
            mean_corr_mat += generate_corr_matrix_rect(clust_dict_list[instance], reverse_dict_list[instance], L)
        mean_corr_mat = mean_corr_mat/n_runs
        
        with open(run_dir+"LIsingB_2D_cmat_"+str(ts)+".pkl", "wb") as fp:   #Pickling
