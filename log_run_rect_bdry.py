...
#sampler = RandomLinDistWidthSampler(a_mat, b_mat, w_mat, Lx*n_runs)

if rank == 0: # The master is the only process that reads the file
	t0 = time.time()
	ts = str(int(100*time.time()))[4:]
	ts+= str(Lx)+str(10*w_bdry)+str(10*w_blk)
	data = [[ts]]*n_processes
...
for item in data:  #Sending to processes
	for inst in range(n_runs):  #Within each process
		...
		zeta_ij_vals = fill_zeta_ij_bdry(Lx*Ly, bdry_dict, adj_ind, a_mat, b_mat, w_mat)
