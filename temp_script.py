...
out_dir = "log_bdry_output/run_time_tests/"
...
input_dict['misc_notes'] = 'Optimized sampling, cache=4*n_runs*L'
sampler = RandomLinDistWidthSampler(a_mat, b_mat, w_mat, 4*n_runs*L)
...
for item in data:  #Sending to processes
	for inst in range(n_runs):  #Within each process
		...
		test = bdry_log_system(L*L, deepcopy(adj_ind), deepcopy(bdry_dict), zeta_ij_vals, beta_vals, track_moments=True)
